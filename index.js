...
client.on('messageCreate', async (message) => {
  ...
  if (message.content === prefix + "help") return message.channel.send({embeds: [new MessageEmbed().setDescription(`**Voici les commandes faisable sur le bot:**\n\n\`${prefix}backup-create\` - **Créé la backup d'un serveur**\n\`${prefix}backup-info (backup id)\` - **Regarde les informations d'une backup**\n\`${prefix}backup-load (backup id)\` - **Charge une backup sur le serveur actuel**\n\`${prefix}backup-delete (backup id)\` - **Supprime une backup (action irréversible)**`).setColor("GREEN")]}).catch(() => false)  
    ...
    backup.fetch(backupip).then((backupInfos) => {
